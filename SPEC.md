...
| Deterministic scheduling | Partial | SCH-01 is implemented in [`src/shared/contracts.ts`](src/shared/contracts.ts), [`src/core/scheduler.ts`](src/core/scheduler.ts), [`src/core/database.ts`](src/core/database.ts), [`src/core/repository.ts`](src/core/repository.ts), [`src/core/service.ts`](src/core/service.ts), [`src/core/api.ts`](src/core/api.ts), and [`src/mcp/server.ts`](src/mcp/server.ts): singleton full-snapshot CAS rules, migration-16 timezone-database diagnostics, persisted-rule-authoritative drafting, host-timezone-independent `Intl` resolution, exact-gap/earlier-overlap warnings, and strict HTTP/MCP contracts. [`tests/scheduler.test.ts`](tests/scheduler.test.ts), [`tests/schedule-rules.test.ts`](tests/schedule-rules.test.ts), [`tests/migrations.test.ts`](tests/migrations.test.ts), and [`tests/persistence.test.ts`](tests/persistence.test.ts) cover DST anomalies, non-hour transitions, host zones, canonicalization, stale/missing revisions, transports, and upgrades. SCH-02 move/publication validation, Content ID warnings, and UI remain pending. |
