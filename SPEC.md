...
| Source-in-place MP4 inventory | Partial | [`src/core/media.ts`](src/core/media.ts) and [`tests/media.test.ts`](tests/media.test.ts) preserve tested source bytes, return per-input groups, reject non-MP4 inputs, and deduplicate canonical paths; watched-folder configuration now has complete repository persistence. The current quick fingerprint is only size plus modification time and is incorrectly treated as a conclusive duplicate, so distinct files can be collapsed without content verification. Best-effort FFmpeg-readable import, watched-folder scanning, missing-source reconciliation, and relinking are pending. |
