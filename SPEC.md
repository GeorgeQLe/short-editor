...
| Candidate generation and review | Partial | [`src/core/candidates.ts`](src/core/candidates.ts), [`src/core/service.ts`](src/core/service.ts), [`src/core/repository.ts`](src/core/repository.ts), [`tests/candidates.test.ts`](tests/candidates.test.ts), [`tests/candidate-integration.test.ts`](tests/candidate-integration.test.ts), [`tests/candidate-corpus.test.ts`](tests/candidate-corpus.test.ts), and the versioned anonymized corpus in [`tests/fixtures/candidate-corpus`](tests/fixtures/candidate-corpus) implement TRC-02: explicit heuristic/analysis selection without fallback, accepted-revision and provider/artifact provenance, complete aligned 20–90 second enumeration, analysis-highlight alignment/expansion, transparent quality floors, stable ranking/grouping, structured insufficiency diagnostics through service/HTTP/MCP, and transactional pending replacement while retaining reviewed rows. The `balanced-beta-v1` corpus gate records 100% validity, recall, precision, and pairwise ranking accuracy against approved 100%/80%/60%/75% thresholds. TRC-03 append/replace strategies and reviewed-decision conflict handling, plus interactive UI/native evidence, remain pending. |
| Revisioned Short projects and invalidation | Partial | [`src/core/repository.ts`](src/core/repository.ts), [`tests/repository.test.ts`](tests/repository.test.ts), [`tests/persistence.test.ts`](tests/persistence.test.ts), and [`tests/transcript-editing.test.ts`](tests/transcript-editing.test.ts) persist source ranges, lineage, composition, captions, audio, and copy; enforce expected revisions; clear approval on render-affecting or accepted-transcript edits; stale prior successful Renders; and invalidate only non-published schedule entries. Complete timeline/editor endpoints and selective copy-only invalidation remain pending. |
