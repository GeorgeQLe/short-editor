...
| Render validation contract | Implemented | RND-02 in [`src/core/render.ts`](src/core/render.ts), [`src/core/repository.ts`](src/core/repository.ts), and migration 13 binds each attempt to an explicit passing preflight and only marks it successful after typed ffprobe validation confirms video/audio streams, 1080×1920 H.264/AAC, positive duration, and the 180-second ceiling. Output, decision/content hashes, graph and encoder provenance, optional sidecar path, failures, cancellation, and stale-revision outcomes are persisted. RND-03 retains normalized frame/audio determinism hashes and repeated-render gating; RND-04 retains retry lineage and advanced recovery. |
