...
| Durable jobs and restart recovery | Partial | [`src/core/jobs.ts`](src/core/jobs.ts), [`src/core/repository.ts`](src/core/repository.ts), [`src/core/artifact-store.ts`](src/core/artifact-store.ts), and their tests implement queueing, cancellation flags, claiming, progress, failure, startup artifact cleanup, cancellation recovery, deduplicated watched-folder/source-reconciliation work, retry of idempotent local jobs, actionable terminal failure for unsafe interrupted cloud-analysis or render work, and bounded supervised cancellation for local transcription. Subprocess cancellation for other workflows and bounded per-workflow retry remain pending. |
...
| Render validation and determinism contract | Implemented | RND-02/RND-03 in [`src/shared/contracts.ts`](src/shared/contracts.ts), [`src/core/render.ts`](src/core/render.ts), [`src/core/repository.ts`](src/core/repository.ts), and migrations 13–14 bind attempts to passing preflights, require typed ffprobe validation, stream-decode canonical `yuv420p` and PCM evidence, and serialize first-baseline or repeated-match completion by a decision/graph/encoder/FFmpeg identity. Mismatches retain validation/evidence, fail with `ARTIFACT_CORRUPT`, and roll back the new MP4, sidecar, and artifact records without modifying prior success. [`tests/render.test.ts`](tests/render.test.ts), [`tests/domain-contracts.test.ts`](tests/domain-contracts.test.ts), [`tests/job-messages.test.ts`](tests/job-messages.test.ts), and [`tests/migrations.test.ts`](tests/migrations.test.ts) cover real repeated rendering, metadata exclusion, independent pixel/sample changes, mismatch cleanup, strict schemas, and every prior migration. RND-04 retains retry lineage and advanced recovery; packaged Windows/release-FFmpeg acceptance remains WIN-03.6. |
