...
| Transcription and vision workers | Partial | [`resources/worker/worker.py`](resources/worker/worker.py), [`src/core/local-transcription.ts`](src/core/local-transcription.ts), and [`src/core/bootstrap.ts`](src/core/bootstrap.ts) implement installed-model-only faster-whisper English transcription, normalized segment/word timing, explicit absent diarization, progress/cancellation, model capability status, and accepted local provenance persistence. [`tests/python-worker-host.test.ts`](tests/python-worker-host.test.ts) and [`tests/local-transcription.test.ts`](tests/local-transcription.test.ts) cover deterministic local-only loading, timing bounds, silence, unsupported input, cancellation, missing models, inventory, and core storage. Ollama, OpenAI, diarization, and vision remain pending; frozen runtime and Windows evidence remain WIN-02/WIN-03 gates. |
