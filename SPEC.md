...
| Local/cloud request boundary | Implemented | [`src/security/credential-vault.ts`](src/security/credential-vault.ts), [`src/electron/main.ts`](src/electron/main.ts), [`src/electron/preload.ts`](src/electron/preload.ts), [`src/core/jobs.ts`](src/core/jobs.ts), [`src/core/service.ts`](src/core/service.ts), and [`src/core/api.ts`](src/core/api.ts) provide OS-protected ciphertext persistence with opaque handles, desktop-only authenticated credential/grant gates, disclosure and revocation, and core verification at both cloud queue and claim time. [`tests/credential-vault.test.ts`](tests/credential-vault.test.ts) and [`tests/cloud-security.test.ts`](tests/cloud-security.test.ts) cover locked storage, plaintext exclusion, forged booleans, mismatched scope, confirmation, revocation, and queued-job races. Native DPAPI behavior and packaged UI validation remain WIN-03 gates. |
| Transcription and vision workers | Partial | [`resources/worker/worker.py`](resources/worker/worker.py), [`src/core/local-transcription.ts`](src/core/local-transcription.ts), [`src/core/local-analysis.ts`](src/core/local-analysis.ts), and [`src/core/bootstrap.ts`](src/core/bootstrap.ts) implement installed-model-only faster-whisper English transcription, schema-constrained Ollama analysis/model discovery, redirect-aware endpoint policy, FFmpeg visual-activity sampling, explicit unsupported detections, normalized cache identity, progress/cancellation, and typed provenance persistence. [`tests/python-worker-host.test.ts`](tests/python-worker-host.test.ts), [`tests/local-transcription.test.ts`](tests/local-transcription.test.ts), [`tests/local-analysis.test.ts`](tests/local-analysis.test.ts), and [`tests/ollama-worker-host.test.ts`](tests/ollama-worker-host.test.ts) cover deterministic local-only loading, timing bounds, provider failures, endpoint classes/redirects, structured-output drift, cache misses, and visual fixtures. OpenAI and diarization remain pending; frozen runtime and Windows evidence remain WIN-02/WIN-03 gates. |
