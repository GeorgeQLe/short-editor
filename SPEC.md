...
- Apple Silicon Macs running macOS 14 or newer are the only v1
  release-acceptance platform.
- Development on other platforms MAY be supported, but results there MUST NOT be
  used to declare v1 complete.
- Distribution MUST use a Developer ID-signed and Apple-notarized arm64
  application in a signed DMG containing the Electron application, local core,
  packaged Python worker runtime, fonts, native SQLite binding, and licensed
...
- Credentials MUST be stored with macOS Keychain-backed protected storage or an
...
### 4.3 macOS packaging and first-run setup
...
- The Python runtime and worker MUST be bundled. A large transcription model
  MUST NOT be bundled and MUST be installed only through an explicit,
  resumable, checksum-verified first-run flow. The UI MUST state size, network,
  license, and privacy implications before transfer.
- Setup Center MUST report FFmpeg/ffprobe, the worker runtime, transcription
  model, optional Ollama, application storage, and relevant permissions using
  application-managed paths and actionable repair controls. Ollama and OpenAI
  MUST remain optional.
- Recovery Center MUST expose bounded retry, relink or return-to-workflow,
  return-to-setup, and attempt history without replacing accepted edits or prior
  successful renders.
- Diagnostic export MUST be previewable and local. It MUST include application,
  platform, dependency, health, migration, and job/render summary state; exclude
  credentials, transcript content, and absolute paths by default; and request
  independent explicit consent for transcript and path inclusion. It MUST NOT
  upload automatically.
- About MUST show the exact application version and supported platform. Beta
  updates MUST use manual signed downloads; v1 MUST NOT include an automatic
  updater.
- The packaging gate MUST exercise installation, launch, import, local analysis,
  edit, render, restart recovery, upgrade, and uninstall on clean supported
  Apple Silicon Macs without development tools.
...
and validation passed, with persisted normalized determinism evidence marked
`baseline` or `matched`.
...
- A Render MUST become `succeeded` only after ffprobe confirms 1080×1920, H.264,
  ...
  seconds.
- After validation, the captured FFmpeg build MUST stream-decode canonical
  1080×1920 `yuv420p` video and signed 16-bit 48 kHz stereo PCM audio into
  SHA-256 hashes and byte counts. Metadata, subtitles, and container structure
  MUST NOT participate.
- The first successful Render for the same decision snapshot, completed
  graph/encoder provenance, normalization version, and FFmpeg build MUST become
  the baseline. Later equivalent attempts MUST match both canonical streams or
  fail with `ARTIFACT_CORRUPT` without exposing their output.
...
- The v1 DST policy identifier is
  `shift-forward-gap-earlier-overlap-v1`. A nonexistent spring-forward wall
  time is shifted forward by the transition's exact offset gap (including
  non-one-hour gaps). An ambiguous fall-back wall time selects the earlier UTC
  instant/offset and reports the later UTC instant as its alternative.
- DST warnings MUST identify the anomaly kind, requested local date and wall
  time, IANA timezone, selected UTC instant, optional alternative UTC instant,
  and adjustment minutes. Only anomalous slots selected into a draft are
  reported.
- The singleton rule set has stable ID `default`. Its first complete snapshot is
  created as revision 1 without `expectedRevision`; every later complete
  replacement requires the exact current revision and increments once.
  Weekdays, wall times, and blackout dates are stored in canonical order, while
  changing the IANA timezone never rewrites configured wall times.
- Each rule write records the runtime timezone-database version. Migration-16
  rules retain `unknown` until their next write. Draft results separately report
  the resolver timezone-database version so offset differences can be
  diagnosed without consulting the host timezone.
- Drafting MUST read the persisted `default` rule set and require
  `expectedRulesRevision`; inline or stale rules MUST NOT create entries.
...
| --- | --- | --- |
| SQLite schema, foreign keys, migrations | Implemented | [`src/core/database.ts`](src/core/database.ts), [`src/core/repository.ts`](src/core/repository.ts), [`tests/migrations.test.ts`](tests/migrations.test.ts), and [`tests/persistence.test.ts`](tests/persistence.test.ts) define and verify ordered transactional migrations, WAL/busy timeout, foreign keys, legacy upgrades, complete entity fields, artifact metadata, template clones, provider provenance, scoped authorization metadata, CAS guards, and rollback after interruption. Native arm64 packaged SQLite remains a release gate. |
| Platform-aware application data path | Implemented | [`src/core/bootstrap.ts`](src/core/bootstrap.ts), [`src/core/startup.ts`](src/core/startup.ts), [`tests/bootstrap.test.ts`](tests/bootstrap.test.ts), and [`tests/startup.test.ts`](tests/startup.test.ts) resolve native paths, create the database/artifacts/logs layout, apply the deterministic four-case native/legacy decision, checkpoint and verify staged migrations, preserve timestamped backups, quarantine failed staging, and refuse both-populated startup without writes. Packaged macOS upgrade validation remains a release gate. |
...
| Source-in-place media inventory | Implemented | [`src/core/media.ts`](src/core/media.ts), [`src/core/watched-folders.ts`](src/core/watched-folders.ts), [`src/core/repository.ts`](src/core/repository.ts), [`tests/media.test.ts`](tests/media.test.ts), and [`tests/reconciliation.test.ts`](tests/reconciliation.test.ts) validate and preserve source media; reconcile enabled folders through debounced events plus durable startup/manual/five-minute scans; enforce root-relative globs and contained file-link rules without directory-link traversal; retain missing Episodes with a safe restore state; automatically repair only SHA-256-verified moves; and require an expiring one-use confirmation for matching no-hash candidates. Packaged Windows watcher, network-drive, and long-path validation remains a release gate. |
| Probe, full hashing, and deduplication | Implemented | [`src/core/media.ts`](src/core/media.ts), [`src/core/bootstrap.ts`](src/core/bootstrap.ts), [`tests/media.test.ts`](tests/media.test.ts), and [`tests/reconciliation.test.ts`](tests/reconciliation.test.ts) verify complete hashing, content-identity deduplication, ambiguity handling, and source immutability. Native release-artifact validation remains a release gate. |
| Durable jobs and restart recovery | Implemented | [`src/core/jobs.ts`](src/core/jobs.ts), [`src/core/repository.ts`](src/core/repository.ts), [`src/core/artifact-store.ts`](src/core/artifact-store.ts), and their tests implement queueing, paired Render/Job cancellation, claiming, progress, bounded failure/reclaim, pair-aware startup reconciliation, artifact cleanup, deduplicated watched-folder/source-reconciliation work, safe retry of idempotent local jobs, actionable terminal recovery for unsafe interrupted work, and cooperative-to-forced media/worker cancellation. Native packaged fault injection remains a release gate. |
| Local/cloud request boundary | Implemented | [`src/security/credential-vault.ts`](src/security/credential-vault.ts), [`src/electron/main.ts`](src/electron/main.ts), [`src/core/openai-provider.ts`](src/core/openai-provider.ts), [`src/core/jobs.ts`](src/core/jobs.ts), [`src/core/service.ts`](src/core/service.ts), and [`src/core/api.ts`](src/core/api.ts) provide OS-protected ciphertext persistence with opaque handles, a typed Electron-to-core bridge, Electron-only OpenAI credential resolution, desktop-only authenticated credential/grant gates, disclosure and revocation, retry-time authorization checks, and core verification before queue, claim, and cache reuse. The core child does not inherit OpenAI credential environment variables. [`tests/credential-vault.test.ts`](tests/credential-vault.test.ts), [`tests/cloud-security.test.ts`](tests/cloud-security.test.ts), and [`tests/openai-core.test.ts`](tests/openai-core.test.ts) cover locked storage, plaintext exclusion, forged booleans, mismatched scope/operation/model, revocation races, and non-secret status. Native Keychain behavior and packaged UI validation remain release gates. |
| Transcription and vision workers | Partial | [`resources/worker/worker.py`](resources/worker/worker.py), [`src/core/local-transcription.ts`](src/core/local-transcription.ts), [`src/core/local-analysis.ts`](src/core/local-analysis.ts), [`src/electron/openai-adapter.ts`](src/electron/openai-adapter.ts), [`src/core/analysis-cache.ts`](src/core/analysis-cache.ts), and [`src/core/bootstrap.ts`](src/core/bootstrap.ts) implement installed-model-only faster-whisper English transcription, schema-constrained Ollama analysis/model discovery, FFmpeg visual-activity sampling, explicit OpenAI transcription/diarization and strict Responses analysis, chunk-scoped speakers, raw/accepted artifacts, bounded retry/cancellation, typed provenance, and exact canonical cache reuse with concurrent-write uniqueness. The local provider suites cover deterministic timing, multi-chunk offsets, provider failures, authorization, provenance, cache misses, and visual fixtures. A signed frozen runtime and model manifest remain release gates. |
| Transcript persistence and replacement | Implemented | [`src/core/repository.ts`](src/core/repository.ts), [`src/core/service.ts`](src/core/service.ts), [`src/core/api.ts`](src/core/api.ts), [`src/mcp/server.ts`](src/mcp/server.ts), and [`tests/transcript-editing.test.ts`](tests/transcript-editing.test.ts) persist immutable accepted revisions; validate full-snapshot text, word, speaker, and timing edits; enforce optimistic conflicts; expose exact/current HTTP and MCP reads; preserve raw artifacts and published schedules; and atomically invalidate dependent analysis, Shorts, Renders, and non-published schedules. Interactive UI is implemented; native packaged proof remains a release gate. |
| Candidate generation and review | Implemented | [`src/core/candidates.ts`](src/core/candidates.ts), [`src/core/service.ts`](src/core/service.ts), [`src/core/repository.ts`](src/core/repository.ts), [`tests/candidates.test.ts`](tests/candidates.test.ts), [`tests/candidate-integration.test.ts`](tests/candidate-integration.test.ts), [`tests/migrations.test.ts`](tests/migrations.test.ts), and the versioned anonymized corpus implement TRC-02/TRC-03: deterministic accepted-revision/provider-bound proposals; explicit append/replace strategies; durable generation runs and Candidate revisions/history; retained-decision and pending-proposal conflict suppression; immutable proposed versus accepted whole content packages; stale-write conflicts; and compatible proposed/accepted Short copy seeding through HTTP/MCP. The `balanced-beta-v1` corpus remains at 100% validity, recall, precision, and pairwise ranking accuracy against approved 100%/80%/60%/75% thresholds. Interactive UI/native evidence remains assigned to UI-01.2/WIN-03.3. |
| Revisioned Short projects and invalidation | Implemented | [`src/shared/contracts.ts`](src/shared/contracts.ts), [`src/core/repository.ts`](src/core/repository.ts), [`src/core/service.ts`](src/core/service.ts), [`src/core/api.ts`](src/core/api.ts), [`src/mcp/server.ts`](src/mcp/server.ts), and [`tests/short-lifecycle.test.ts`](tests/short-lifecycle.test.ts) implement EDT-01: guarded revision-1 creation with independent accepted-transcript captions; strict bounded integer-millisecond ranges; atomic timeline and approval CAS; accepted-copy approval; exact revision increments; render-affecting invalidation; copy/title-only preservation; and byte-preserved published schedules through HTTP and MCP. Interactive editor evidence exists; packaged macOS evidence remains a release gate. |
...
| Caption editing, validation, and sidecars | Implemented | [`src/shared/contracts.ts`](src/shared/contracts.ts), [`src/core/captions.ts`](src/core/captions.ts), [`src/core/artifact-store.ts`](src/core/artifact-store.ts), [`src/core/repository.ts`](src/core/repository.ts), [`src/core/service.ts`](src/core/service.ts), [`src/core/api.ts`](src/core/api.ts), and [`src/mcp/server.ts`](src/mcp/server.ts) implement EDT-04: independent cue/word timing, approved packaged Inter styles, real OpenType metrics and glyph coverage, persisted typed layout/range warnings, exact-CAS invalidation, atomic revision-owned UTF-8 SRT/WebVTT sidecars, and HTTP/MCP parity. [`tests/captions.test.ts`](tests/captions.test.ts), [`tests/caption-service.test.ts`](tests/caption-service.test.ts), [`tests/migrations.test.ts`](tests/migrations.test.ts), and [`tests/artifact-store.test.ts`](tests/artifact-store.test.ts) cover layout, timing remapping, encoding, migration, rollback, transcript independence, and lifecycle behavior. Interactive controls remain UI-01.3; burned-in composition remains RND-02. |
| Audio editing | Implemented | EDT-05 is implemented across [`src/shared/contracts.ts`](src/shared/contracts.ts), [`src/core/audio.ts`](src/core/audio.ts), [`src/core/database.ts`](src/core/database.ts), [`src/core/repository.ts`](src/core/repository.ts), [`src/core/service.ts`](src/core/service.ts), [`src/core/api.ts`](src/core/api.ts), and [`src/mcp/server.ts`](src/mcp/server.ts): strict bounded audio state and migration 11; Episode-only contiguous source routing; deterministic capped edge fades; continuous zero-offset bed loop/trim decisions; typed 12 dB speech/background warnings; exact-CAS Render/schedule invalidation; and HTTP/MCP parity. [`tests/audio.test.ts`](tests/audio.test.ts), [`tests/migrations.test.ts`](tests/migrations.test.ts), and [`tests/short-lifecycle.test.ts`](tests/short-lifecycle.test.ts) cover the engine, compatibility, lifecycle, and transports. RND-01 now captures and validates these decisions; actual FFmpeg mixing remains RND-02 and interactive controls remain UI-01.3. |
| Render preflight | Implemented | RND-01 is implemented by strict public contracts and the registry in [`src/shared/contracts.ts`](src/shared/contracts.ts), canonical stable probing and decision capture in [`src/core/render-preflight.ts`](src/core/render-preflight.ts), immutable migration 12 and insert/read-only repository operations, an atomic final revision check, and matching HTTP/MCP operations. [`tests/render-preflight.test.ts`](tests/render-preflight.test.ts) covers canonical hashes, registry ordering, dependency parsing, exact/over duration boundaries, Content ID help, stale and concurrent revisions, database immutability, response redaction, repeatability, and the no-Render/job/artifact invariant; [`tests/migrations.test.ts`](tests/migrations.test.ts) upgrades every prior schema. RND-02 still owns FFmpeg graph construction and output creation. |
| Render validation, retry, and determinism contract | Implemented | RND-02–RND-04 in [`src/shared/contracts.ts`](src/shared/contracts.ts), [`src/core/render.ts`](src/core/render.ts), [`src/core/repository.ts`](src/core/repository.ts), and migrations 13–15 bind attempts to passing preflights, expose immutable three-attempt lineage, require typed ffprobe validation, stream-decode canonical `yuv420p` and PCM evidence, and serialize first-baseline or repeated-match completion by a decision/graph/encoder/FFmpeg identity. Manual retry reuses the exact revision/preflight/sidecar snapshot; cancellation and recovery clean the new attempt without modifying prior success. Tests cover real repeated rendering, retry lineage/limits, cancellation, metadata exclusion, pixel/sample changes, mismatch cleanup, and strict schemas. Packaged release-FFmpeg acceptance remains a release gate. |
...
| Frozen release interface and diagnostics policy | Implemented | API-03 freezes the exact generated HTTP inventory and MCP Draft-07 schemas with [`docs/release-interface-v1.json`](docs/release-interface-v1.json), publishes the release-facing [`docs/release-interfaces-v1.md`](docs/release-interfaces-v1.md), and proves every MCP tool maps to one matching HTTP operation. [`src/shared/diagnostics.ts`](src/shared/diagnostics.ts) defines `diagnostic-export-v1`: credential keys and recognizable credential strings are always removed, while transcript/source/path fields require explicit opt-in. [`tests/api-release-contract.test.ts`](tests/api-release-contract.test.ts) prevents artifact, digest, documentation, mapping, or redaction drift; [`tests/api-contract.test.ts`](tests/api-contract.test.ts) also proves stable-ID traversal when rows are inserted between pages. |
| Electron/React workflow UI | Implemented | [`src/ui/App.tsx`](src/ui/App.tsx) and the workspace components provide Library, Candidate, editor, render, calendar, cloud-consent, Setup, Recovery, diagnostics, and About workflows. Component and workflow suites cover persisted actions; signed-artifact acceptance remains a release gate. |
| Accessibility | Partial | Primary controls are semantic and keyboard-native, focus is visible, status changes are announced, state has text and shape alternatives, reduced motion is honored, and numeric timeline/crop inputs remain available. A release-candidate VoiceOver, contrast, focus-restoration, and 200% scaling walkthrough remains required. |
| macOS distribution | Partial | [`package.json`](package.json) defines an arm64 DMG, hardened runtime, macOS 14 baseline, and packaged-resource boundary. [`resources/runtime-manifest.json`](resources/runtime-manifest.json) is the auditable runtime inventory. Signing, notarization, staged runtime checksums, and clean-Mac acceptance must be completed for the exact release candidate. |

## 9. Release gates

v1 is complete only when every MUST in this document is implemented and all gates
below pass on macOS 14 and newer against a representative beta corpus. A gate MAY contain
...
- Import a mixed batch containing guaranteed MP4, best-effort readable media,
  ...
  long and Unicode macOS paths, read-only sources, zero-byte files, video without audio,
...
- On clean supported Apple Silicon Macs, install without development tools, launch,
...
- Changed v1 release acceptance to a direct-download, signed and notarized
  Apple Silicon application for macOS 14+; Windows packaging moved beyond v1.
- Reconciled completed hashing, deduplication, rendering, and React workflow
  evidence while retaining all v1 non-goals and frozen HTTP/MCP interfaces.
- Added application-owned setup, recovery, local diagnostic export, About, and
  manual-update requirements. Large model transfer remains explicit and must
  preserve a prior verified installation.

### 1.5.0 — 2026-07-29

- Froze the exact HTTP inventory and MCP input/output schemas behind a
  digest-bearing v1 compatibility manifest and deterministic generated release
  guide.
- Added exact generated-artifact and HTTP-to-MCP mapping regressions plus
  stable-ID pagination coverage for inserts between pages.
- Centralized `diagnostic-export-v1`, which always removes credential material
  and requires explicit opt-in for transcript, source, and path detail.
...
- Implemented durable Candidate generation runs, append/replace regeneration,
  optimistic Candidate revisions, retained-decision conflict suppression, and
  immutable proposed versus accepted Candidate content packages.
- Preserved legacy Short copy as accepted and added explicit copy state/source
  metadata for Candidate-derived drafts and user-accepted edits.
