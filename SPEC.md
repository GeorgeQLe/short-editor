...
| Revisioned Short projects and invalidation | Partial | [`src/core/repository.ts`](src/core/repository.ts), [`tests/repository.test.ts`](tests/repository.test.ts), [`tests/persistence.test.ts`](tests/persistence.test.ts), and [`tests/transcript-editing.test.ts`](tests/transcript-editing.test.ts) persist source ranges, lineage, composition, captions, audio, complete copy, and proposed/accepted copy source metadata; enforce expected revisions; clear approval on render-affecting or accepted-transcript edits; stale prior successful Renders; and invalidate only non-published schedule entries. Candidate-accepted copy now seeds accepted Shorts while unaccepted proposals seed explicitly proposed drafts. Complete timeline/editor endpoints and selective copy-only invalidation remain pending. |
